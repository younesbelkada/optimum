...
if __name__ == "__main__":
    ...
    if args.is_decoder:
        ...
        hf_model = AutoModelForCausalLM.from_pretrained(args.model_name, torch_dtype=torch.float16 if args.use_half else None, use_cache=False).eval()
    else:
        hf_model = AutoModel.from_pretrained(args.model_name, torch_dtype=torch.float16 if args.use_half else None).eval()
